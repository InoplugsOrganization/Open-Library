...
class WC_Add_Fees
{
	...
	/**
	 * Flag if WC uses HPOS tables
	 *
	 * @since 4.0
	 * @var boolean
	 */
	protected $wc_hpos_support;

	/**
	 * Flag set when in callback of block checkout page to recalculate fees
	 *
	 * @since 5.0
	 * @var boolean
	 */
	protected $in_block_checkout_callback;
...
//		$this->dp                = (int) get_option( 'woocommerce_price_num_decimals' );
		...
		$this->wc_hpos_support = false;
		if( class_exists( '\Automattic\WooCommerce\Utilities\OrderUtil' ) )
		{
			$this->wc_hpos_support = \Automattic\WooCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled();
		}
		$this->in_block_checkout_callback = false;
...
//		add_action( 'woocommerce_init', array( $this, 'handler_wc_init' ), 500 );
	...
	/**
	 * Returns if WooCommerce uses HPOS tables
	 *
	 * @since 4.0
	 * @return boolean
	 */
	public function wc_hpos_support_active()
	{
		return $this->wc_hpos_support;
	}
	...
	public function handler_wp_register_scripts()
	{
		...
		//	@since 5.1.2  WC 9.6 breaks checkout page when 'wc-blocks' is used
//		wp_register_script( 'wc_additional_fees_block_script', self::$plugin_url . 'js/wc_additional_fees_block_checkout.js', array( 'wp-data', 'wc-blocks', 'wc_additional_fees_script' ), true );
		wp_register_script( 'wc_additional_fees_block_script', self::$plugin_url . 'js/wc_additional_fees_block_checkout.js', array( 'wp-data', 'wc_additional_fees_script' ), true );
	}
	...
	public function handler_wp_print_styles()
	{
		$var = array(
				...
				'add_fee_loader'	=> self::$plugin_url . 'images/loading.gif',
		...
		/**
		 * @since 5.0
		 * @param boolean $load_scripts
		 * @return boolean					anything except true will disable block cart and block checkout support
		 */
		if( true === apply_filters( 'wc_add_fees_load_block_checkout_scripts', true ) )
		{
			if( class_exists( 'Automattic\WooCommerce\Blocks\Utils\CartCheckoutUtils' ) )
			{
				if( Automattic\WooCommerce\Blocks\Utils\CartCheckoutUtils::is_checkout_block_default() )
				{
					wp_enqueue_script( 'wc_additional_fees_block_script' );
				}
			}
		}
	}
	 ...
	 *
	 * @deprecated 4.0				removed handler
	 * @param int $post_ID
	 * @param WP_Post $post
	 * @param bool $update
	 */
	 ...
	 *
	 * @since 5.0		support for WC Block Checkout page
	 */
	protected function attach_to_woocommerce()
	{
		 ...
		 */
		add_filter( 'woocommerce_calculated_total', array( $this, 'handler_wc_calculated_totals' ), 50000, 2 );
		...
		{
			/**
			 * Load callback when in frontend payment gateway changes to recalculate our fees
			 *
			 * @since 5.0
			 */
			if( did_action( 'woocommerce_blocks_loaded' ) )
			{
				$this->handler_wc_blocks_loaded();
			}
			else
			{
				add_action( 'woocommerce_blocks_loaded', array( $this, 'handler_wc_blocks_loaded' ), 50000 );
			}
		}
...
//		add_action( 'woocommerce_subscription_renewal_payment_failed',  array( $this, 'handler_wc_subscription_renewal_payment_failed' ), 10, 2 );
	...
	static public function &get_options_default()
	{
		...
		if( isset( self::$_instance) && ( count( WC_Add_Fees::instance()->gateways) > 0 ) )
		{
			foreach ( WC_Add_Fees::instance()->gateways as $key => $gateway )
			{
				...
				$go = self::get_option_gateway_default( $option_gateway, $gateway->get_title() );
	...
	static public function &get_post_meta_product_default( $post_id )
	{
		...
		if( isset( self::$_instance ) && ( count( WC_Add_Fees::instance()->gateways ) > 0) )
		{
			...
			{
				$option_gateway = array();
				$go = self::get_option_gateway_default( $option_gateway, $gateway->get_title(), true );
				$default[WC_Add_Fees::OPT_GATEWAY_PREFIX][ $key ] = $go;
			}
		}
	...
	static public function &get_post_meta_order_default( $post_id )
	{
		...
		if( ! WC_Add_Fees::instance()->wc_hpos_support_active() )
		{
		...
		else
		{
			...
			$pm = $order->get_meta( self::KEY_POSTMETA_ORDER, true );
		}
		...
		if( $old_opt != $new_opt )
		{
			...
			{
				$order->update_meta_data( self::KEY_POSTMETA_ORDER, $new_pm );
				$order->save();
			}
		}
	...
	static public function &get_option_gateway_default( array $option_gateway, $gateway_name = '', $for_postmeta = false )
	{
		...
		$default = array(
					...
					WC_Add_Fees::OPT_KEY_OUTPUT				=> esc_html( $text ),
	...
	public function handler_wc_generated_manual_renewal_order( $order_id )
	{
		...
		foreach ( $order->get_fees() as $index => $fee )
		{
			$meta = $fee->meta_exists( '_added_by' ) ? $fee->get_meta( '_added_by', true ) : '';

			$remove = $meta == self::OPTIONNAME;
			 ...
			 * @param int $index
			 * @param int $order_id
			 * @return boolean					true to remove fee
			 */
			$remove = apply_filters( 'wc_add_fees_subscriptions_remove_fees', $remove, 'order_id', $fee, $index, $order_id );
...
//		$order->save();
	...
	public function handler_wc_adjust_order_fees_for_subscription_initial_payment( WC_Abstract_Order $order, WC_Cart $cart )
	{
		foreach ( $order->get_fees() as $fee )
		{
			...
			/**
			 * If "our" fees are added manually they are not recognised and therefore duplicated on renewal
			 *
			 * @since v3.2.4
			 * @param boolean $remove
			 * @param string $context
			 * @param WC_Order_item_Fee $fee
			 * @param int $deprecated				removed 4.0.1
			 * @param WC_Abstract_Order $order
			 * @param WC_Cart $cart
			 * @return boolean					true to remove fee
			 */
			$remove = apply_filters( 'wc_add_fees_subscriptions_remove_fees', $remove, 'order_cart', $fee, 0, $order, $cart );
	...
	/**
	 * Register callback for block checkout page
	 *
	 ...
	 * @since 5.0
	 */
	public function handler_wc_blocks_loaded()
	{
		$args = [
				'namespace' => 'wc_additional_fees_wc_block_payment_gateway_changed',
				'callback'  => [ $this, 'cb_block_payment_gateway_changed' ]
			];
	...
	/**
	 * Set the choosen payment gateway.
	 * The callback to calculate cart total is done by REST API Handler. We MUST NOT call it.
	 * Triggered by extensionCartUpdate() in frontend
	 *
	 * @since 5.0
	 * @param array $data
	 * @return void
	 */
	public function cb_block_payment_gateway_changed( $data )
	{
		if( isset( $data['add_fee_paymethod'] ) )
		{
			WC()->session->set( 'chosen_payment_method', $data['add_fee_paymethod'] );
			$this->in_block_checkout_callback = true;
		}
	}

	/**
	 * This checks (or tries to check) if we are on the block checkout page
	 *
	 * @since 5.0
	 * @return boolean
	 */
	protected function rest_api_calculate_totals_check_to_continue()
	{
		...
		/**
		 * true when payment gateways changed on block checkout page
		 */
		if( $this->in_block_checkout_callback )
		{
			return true;
		}
		...
		/**
		 * Proceed with requests not from cart
		 */
		if( false === stripos( $uri, '/cart/' ) )
		{
			return true;
		}

		/**
		 * Filter route and return to continue calculation of fees or not
		 * Same routes for block cart and block checkout page e.g.:
		 *		"/wp56/wp-json/wc/store/v1/cart/select-shipping-rate?_locale=user"
		 *
		 * @since 5.0
		 * @param null $is_block_checkout_route
		 * @return boolean|null				false | true to short circuit, null to continue with our default checks
		 */
		$is_block_checkout_route = apply_filters( 'wc_add_fees_rest_api_route_is_block_checkout_route', null );

		if( true === $is_block_checkout_route || false === $is_block_checkout_route )
		{
			return $is_block_checkout_route;
		}

		/**
		 * This might not exist for REST_API requests
		 */
		$referer = isset( $_SERVER['HTTP_REFERER'] ) ? $_SERVER['HTTP_REFERER'] : '';
		if( '' != trim( $referer ) )
		{
			if( $referer == wc_get_checkout_url() )
			{
				return true;
			}
	...
	{
		static $done = 0;
		static $original_total = null;
		static $automated_taxes = false;

		if( is_null( $original_total ) )
		{
			$original_total = $total;
			$automated_taxes = class_exists( 'WC_Connect_TaxJar_Integration', false ) && ( get_option( 'wc_connect_taxes_enabled', 'no' ) == 'yes' );
		}

		if( ! $this->rest_api_calculate_totals_check_to_continue() )
		{
			return $total;
		}

		$calculate = is_checkout() || defined( 'WOOCOMMERCE_CHECKOUT' ) || $this->in_block_checkout_callback || defined( 'REST_REQUEST' );
		if( ! $calculate )
		{
			return $total;
		}
		...
		{
			/**
			 * If totals are different then we are in second call for calculated_totals.
			 * As automated tax calculation relies on "Prices entered without tax" our first fee calculation is correct and in second call
			 * our fees become correct tax added by tax plugin.
			 *
			 * @since 5.2
			 */
			if( abs( $original_total - $total ) > 0.00001 )
			{
				$done = 0;
				return $total;
			}
		...
		/**
		 * @since 4.1
		 * @since 5.2			no longer necessary when WC Tax and Shipping used, will be removed in one of the next releases, added $original_total, $total, $done
		 * @param boolean $check
		 * @param string $context
		 * @param float $original_total
		 * @param float $total
		 * @param int $done
		 * @return boolean
		 */
		if( false !== apply_filters( 'wc_add_fees_wc_calculated_totals_done_check', false, 'handler_wc_calculated_totals', $original_total, $total, $done ) )
		{
			if( $done > 0 )
			{
				return $total;
		...
		if( ! empty( $this->payment_gateway_option['mails_no_fee'] ) )
		{
			$checkout = WC()->checkout();
			$fields = $checkout->get_checkout_fields( 'billing' );
			$email = $checkout->get_value( 'billing_email' );

			if( $this->check_exclude_email_from_fee( $email, $this->payment_gateway_option['mails_no_fee'] ) )
			{
				return $total;
			}
		}
		...
		/**
		 * Fixes problem with block checkout to display taxes correctly
		 *
		 * @since 5.2
		 */
		$obj_wc_cart->set_total_tax( $obj_wc_cart->get_total_tax() + $fee_sum_tax );
...
//								),
	...
	public function handler_wc_resume_order( $order_id )
	{
		...
		{
			$order = wc_get_order( $order_id );
			$order->delete_meta_data( self::KEY_POSTMETA_ORDER );
			$order->save();
		}
	}
	...
	public function handler_ajax_calc_fee_pay_order()
	{
		...
		$response ['alert'] = __( 'An error occurred in the calculation of fees for your selected payment gateway. Kindly contact us to recheck your invoice or try to use another payment gateway. ', self::TEXT_DOMAIN );
		...
		/**
		 * To allow pay_for_order for non logged in users see:
		 * https://github.com/InoplugsOrganization/Open-Library/blob/main/Payment%20Gateway%20Based%20Fees/Pay%20for%20order/wc_add_fees_order_pay_without_login.php
		 *
		 * @since 4.0
		 */
	 ...
	 *
	 * @since 5.3		changed calculation to avoid 0,01 rounding errors with WooPayment (and maybe other payment gateways)
	 *					rounded "no tax" value only and then recalculated taxes again leaving decimals there
	 *
	...
	protected function calculate_fees( $cart_includes_tax, $value, array $gateway, $quantity = 1, $tax_rates_base = array() )
	{
		...
		/**
		 * Support for Deposits plugin
		 *
		 * @since 3.2.5
		 */
		if( class_exists( 'WC_Deposits_Cart_Manager', false ) )
		{
			if( isset( WC()->cart ) && WC()->cart instanceof WC_Cart )
			{
				$cart_manager = WC_Deposits_Cart_Manager::get_instance();

				if( $cart_manager->has_deposit( WC()->cart ) )
				{
					if( $cart_includes_tax )
					{
						$future_payments = $cart_manager->get_future_payments_amount();
					}
					else
					{
						$future_payments = $cart_manager->get_future_payments_amount_no_tax();
					}

					$value -= $future_payments;
		...
		$add_fee_maximum = apply_filters( 'wc_add_fees_gateway_fee_maximum', (float) $gateway['maximum_fee'], $gateway, $value );
		...
		//	calculate $add_fee_minimum amount according to setting: Prices entered w/o tax
		if( $add_fee_minimum > 0.0 )
		{
			...
			if( $prices_include_tax )
			{
				//	recalculate tax based on rounded value
				$add_fee_minimum_no_tax = round( $add_fee_minimum - $tax_amount_minimum, $this->dp );

				$taxes_add_minimum_rounded = WC_Tax::calc_tax( $add_fee_minimum_no_tax, $tax_rates, false );
				$tax_amount_minimum = WC_Tax::get_tax_total( $taxes_add_minimum_rounded );
			}
			else
			{
				$add_fee_minimum_no_tax = $add_fee_minimum;
		...
		$add_fee_maximum_no_tax = $add_fee_maximum;
		$add_fee_maximum_tax = $add_fee_maximum;
		$tax_amount_maximum = 0.0;
		$taxes_add_maximum = array();

		//	calculate $add_fee_maximum amount according to setting: Prices entered w/o tax
		if( $add_fee_maximum > 0.0 )
		{
			$taxes_add_maximum = WC_Tax::calc_tax( $add_fee_maximum, $tax_rates, $prices_include_tax );
			$tax_amount_maximum = WC_Tax::get_tax_total( $taxes_add_maximum );

			if( $prices_include_tax )
			{
				//	recalculate tax based on rounded value
				$add_fee_maximum_no_tax = round( $add_fee_maximum - $tax_amount_maximum, $this->dp );

				$taxes_add_maximum_rounded = WC_Tax::calc_tax( $add_fee_maximum_no_tax, $tax_rates, false );
				$tax_amount_maximum = WC_Tax::get_tax_total( $taxes_add_maximum_rounded );
			}
			else
			{
				$add_fee_maximum_no_tax = $add_fee_maximum;
			...
			$add_fee_maximum_tax = $add_fee_maximum_no_tax + $tax_amount_maximum;
		}
		else
		{
			$add_fee_maximum_no_tax = $add_fee_maximum_tax = $tax_amount_maximum = 0.0;
		}
		...
		/**
		 * Check for a maximum fee
		 */
		if( $add_fee_maximum > 0.0 )
		{
			if( $tax_included )
			{
				if( $add_fee > ( $add_fee_maximum_tax + $add_fee_fixed_tax ) )
				{
					$add_fee = $add_fee_maximum_tax;
				}
			}
			else
			{
				if( $add_fee > ( $add_fee_maximum_no_tax + $add_fee_fixed_no_tax ) )
				{
					$add_fee = $add_fee_maximum_no_tax;
				}
			}
		}
		...
		switch ( $gateway[self::OPT_KEY_ADD_VALUE_TYPE] )
		{
			...
			case self::VAL_INCLUDE_PERCENT:
				if( $tax_included )
				{
					...
					$tax_amount_incl = WC_Tax::get_tax_total( $taxes );
					$add_fee = $add_fee - $tax_amount_incl;
				}

				$add_fee = round( $add_fee + $add_fee_fixed_no_tax, $this->dp );

				$taxes = WC_Tax::calc_tax( $add_fee, $tax_rates, false );
...
//				break;
	...
	{
		/**
		 * User reported  "Trying to access array offset on value of type null" for very small values like 0.05
		 * $fee->gateway_option is null if fee amount = 0
		 * in this case we can skip to add fees and avoid fee line with 0
		 *
		 * @since 4.2
		 */
		if( ! isset( $fee->gateway_option[WC_Add_Fees::OPT_KEY_OUTPUT] ) )
		{
			return;
		}

		if( ! isset( $fee->gateway_option[WC_Add_Fees::OPT_KEY_TAXCLASS] ) )
		{
			$fee->gateway_option[WC_Add_Fees::OPT_KEY_TAXCLASS] = self::VAL_TAX_STANDARD;
		}
	...
	public function load_request_data( $posted_payment_gateway = '' )
	{
		...
		{
			/**
			 * Encountered in dev environment, that $posted_payment_gateway == null
			 *
			 * @since 5.2
			 */
			$posted_payment_gateway = '';
		}
		...
		//
		// @since 3.1.13 we remove the first part as this breaks several 3-rd party plugins
//		if( isset( WC()->cart ) && ( ! $this->gateway_bugfix ) )
		...
		/**
		 * Possible Bugfix with Klarna Payments Gateway:
		 *
		 * On checkout it seems to return "klarna_payments_pay_later" instead of "klarna_payments" which is the actual key - seen on a user site, but unable to activate plugin on my site
		 * So we check for gateway key and replace $posted_payment_gateway with this key to get option
		 *
		 * @since 3.2.2
		 */
		if( 0 === strpos( $posted_payment_gateway, 'klarna_payments' ) )
		{
			$posted_payment_gateway = 'klarna_payments';
		}
		/**
		 * =====   end bugfix
		 */
	...
	protected function calculate_gateway_fee_total ( $includes_tax, $total_excl, $total_incl, $tax_rates_base = array() )
	{
		...
		$fees_calc->gateway_title = $this->gateways[ $this->payment_gateway_key ]->get_title();
	...
	public function calculate_gateway_fees_order( $order_id, WC_Order_Add_Fees &$add_fee_order, $ignore_recalc_option = false )
	{
		...
		if( ! empty( $this->payment_gateway_option['mails_no_fee'] ) )
		{
			...
			$check_mails = $this->payment_gateway_option['mails_no_fee'];

			if( $this->check_exclude_email_from_fee( $billing_mail, $check_mails ) )
			{
				$allow_fees = false;
			}
		}
		...
		{
			$order->update_meta_data( self::KEY_POSTMETA_ORDER, $pm );
		 ...
		 *
		 * @since ????
		 * @param boolean $allow_fees
		 * @param WC_Order $order
		 * @return boolean
		 */
		if( ! apply_filters( 'wc_add_fees_order_before_add_total_fee', $allow_fees, $order ) )
		{
			$title = in_array( $this->payment_gateway_key, array( 'other', '' ) ) ? $this->payment_gateway_key : $this->gateways[ $this->payment_gateway_key ]->get_title();
	...
	/**
	 * Checks if $email is contained in $mails_no_fee
	 *
	 * @since 4.1
	 * @param string $email
	 * @param string $mails_no_fee
	 * @return bool
	 */
	protected function check_exclude_email_from_fee( $email, $mails_no_fee )
	{
		if( empty( $email ) || empty( $mails_no_fee ) )
		{
			return false;
		}
		...
		{
			if( trim( strtolower( $value ) ) == $email )
			{
				return true;
			}
		}
