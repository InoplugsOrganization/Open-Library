...
//		wc_add_fees_payment_selected_id = $(this).attr('id');
		...
		let current_selected_id = $( '.payment_methods .input-radio:checked' ).attr('id');
		let changed_id = $(this).attr('id');

		if( changed_id == current_selected_id )
		{
			$('body').trigger('update_checkout');
		}
		else
		{
			// added 5.2.2 - fixes a problem when checkbox change is triggered programmatically by 3rd party plugin like WooCommerce Square
			setTimeout( function()
			{
				$('body').trigger('update_checkout');
			}, 100 );
