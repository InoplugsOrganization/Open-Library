...
//		$order->save();
	...
	public function handler_wc_calculated_totals( $total, WC_Cart $obj_wc_cart )
	{
		...
		if( $automated_taxes )
		{
			 ...
			 * @since 5.1.5
			 */
			if( round( $total, 2 ) != $original_total )
			{
				$done = 0;
				return $total;
			}
		 ...
		 * @return boolean
		 */
		if( false !== apply_filters( 'wc_add_fees_wc_calculated_totals_done_check', false, 'handler_wc_calculated_totals' ) )
		{
			if( $done > 0 )
			{
				return $total;
...
//		$tax_rates = $obj_wc_tax->get_rates( $taxclass );
		...
		if( $add_fee_maximum > 0.0 )
		{
			...
			if( ! $this->round_at_subtotal )
			{
				$tax_amount_maximum = round( $tax_amount_maximum, $this->dp );
			}

			if( $prices_include_tax )
			{
				$add_fee_maximum_no_tax = $add_fee_maximum - $tax_amount_maximum;
			}
			else
			{
				$add_fee_maximum_no_tax = $add_fee_maximum;
